"""
Step 13: EMA 标准路网 - 纯用户侧优化
=====================================
与 Step 12（voltage）对照，只用 baseline reward：
    r = -(queue_h + trip_h) / 2
不包含任何电网/电压信号，作为消融对照组。

用法:
    python tests/step13_ema_user_only.py --mode dry-run
    python tests/step13_ema_user_only.py --mode train
    python tests/step13_ema_user_only.py --mode eval
"""
...
CHECKPOINT_BASENAME = f"step13_ema_user_seed{EXPERIMENT_SEED}"
...
def _build_cfg():
    return TrainConfig(
        ...
        station_id_key="",
        graph_group="l0",
        train_scale="step13_ema_user",
        base_seed=EXPERIMENT_SEED,
        output_dir="runs/step13_ema_user",
...
def run_dry_run():
    print("=" * 62)
    print(f"  Step 13 dry-run: EMA (74 nodes, 3 stations) [reward={REWARD_MODE}]")
    print("=" * 62)
    ...
    print(f"\n  [Note] training reward: r = -(queue_h + trip_h) / 2  (no grid/voltage)")
    print(f"  [Dry-run OK] 50 steps, {total_decisions} decisions")
...
def run_train():
    cfg = _build_cfg()
    print("=" * 62)
    print(f"  Step 13 train: EMA user-only, {EPISODES}ep × {STEPS_PER_EPISODE}step")
    print(f"  reward = -(queue_h + trip_h) / 2,  clip ∈ [-1, 0]")
    print("=" * 62)
...
def run_eval():
    print("=" * 62)
    print("  Step 13 eval: EMA user-only [reward=baseline]")
    print("=" * 62)
    ...
    for policy in ["random", "greedy", "fed"]:
        ...
        results[policy] = _evaluate_policy(
            policy, CHECKPOINT_BASENAME if policy == "fed" else None
        )
    ...
    print(f"\n{'=' * 78}")
    print(f"  Step 13 Results: EMA user-only (74 nodes, 3 stations)")
    print(f"{'=' * 78}")
    ...
    path = os.path.join(RUN_ROOT, "step13_ema_user_eval.json")
...
# ── main ─────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Step 13: EMA 纯用户侧优化")
